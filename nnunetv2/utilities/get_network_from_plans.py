...
def get_network_from_plans(plans_manager: PlansManager,
    ...
    mapping = {
        ...
        'StenUNetPretrained': StenUNetPretrained
    }
    kwargs = {
        'PlainConvUNet': {
            'conv_bias': True,
    ...
    # network class name!!
    if segmentation_network_class_name == 'StenUNetPretrained':
        model = StenUNetPretrained(
            num_input_channels=num_input_channels,
            n_stages=num_stages,
            features_per_stage=[min(configuration_manager.UNet_base_num_features * 2 ** i,
                                    configuration_manager.unet_max_num_features) for i in range(num_stages)],
            conv_op=conv_op,
            kernel_size=configuration_manager.conv_kernel_sizes[0],  # just pick one, doesn't affect pretrained encoder
            strides=configuration_manager.pool_op_kernel_sizes,
            blocks_per_stage_encoder=configuration_manager.n_conv_per_stage_encoder,
            blocks_per_stage_decoder=configuration_manager.n_conv_per_stage_decoder,
